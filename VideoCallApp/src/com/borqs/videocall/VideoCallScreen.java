...
public final class VideoCallScreen extends Activity implements TextureView.SurfaceTextureListener {
	...
	// vtstatus 
	private static final String vtstatusPath = "/data/tmp/vtstatus";
	...
	private void updateCallInfo(){
		...
		if (VideoCallApp.sPanelMode == VTPanel.VTPANEL_MODE_ANSWERIN_CONNECTING    
			...
			mSlidingManager.updateCallerInfo((String)getText(R.string.answerin_popup_call_card_title), strNumber, cityName, strName);
		if(VideoCallApp.sPanelMode == VTPanel.VTPANEL_MODE_DAILOUT_CONNECTING
				&& mSlidingManager!=null)
 			mSlidingManager.updateCallerInfo((String)getText(R.string.dailout_popup_call_card_title), strNumber, cityName, strName);
		else
	...
	private void UpdateUIAfterConnecting() {
		...
		//        if(mRingingMuteView!=null)
			//        {
			//        	mRingingMuteView.setVisibility(View.GONE);
		//        }
...
//					txtFallback.setText(R.string.local_viedeo_open)	;
		...
		mChkFallBack = (Button)findViewById(R.id.btnvoice);
		mChkFallBack.setOnClickListener(new Button.OnClickListener() {

			public void onClick(View v) {
				if (isVoiceCall == false)
				{
					isVoiceCall = true;
					if(mSlidingManager.mIncomingSelector != null)
					{
						mSlidingManager.mIncomingSelector.setLeftTabResources(R.drawable.ic_answer_voice_call,
								R.drawable.cmcc_phone_target_green,
								R.drawable.jog_tab_bar_left_answer,
								R.drawable.jog_tab_left_answer);
						mSlidingManager.mIncomingSelector.setRightTabResources(R.drawable.ic_reject_voice_call,
							R.drawable.cmcc_phone_target_red,
							R.drawable.jog_tab_bar_right_decline,
							R.drawable.jog_tab_right_decline);
					}
					mChkFallBack.setBackgroundResource(R.drawable.accept_as_voice_pressed);
					mChkFallBack.setText(R.string.unlock_fall_back);
				}
				else
				{
					isVoiceCall = false;
					if(mSlidingManager.mIncomingSelector != null)
					{
						mSlidingManager.mIncomingSelector.setLeftTabResources(R.drawable.ic_answer_video_call,
								R.drawable.cmcc_phone_target_green,
								R.drawable.jog_tab_bar_left_answer,
								R.drawable.jog_tab_left_answer);
						mSlidingManager.mIncomingSelector.setRightTabResources(R.drawable.ic_reject_video_call,
							R.drawable.cmcc_phone_target_red,
							R.drawable.jog_tab_bar_right_decline,
							R.drawable.jog_tab_right_decline);
					}
					mChkFallBack.setBackgroundResource(R.drawable.accept_as_voice);
					mChkFallBack.setText(R.string.lock_fall_back);
				}
			}
		});
		updateCameraUI();
	...
	private void setupViews() {
		...
		//   mRingingMuteView= (ImageView)findViewById(R.id.img_ringing_mute);
		...
		if (VideoCallApp.sPanelMode == VTPanel.VTPANEL_MODE_DAILOUT_CONNECTING){ 
			...
			// mRingingMuteView.setVisibility(View.INVISIBLE);
		...
		if( VideoCallApp.sPanelMode == VTPanel.VTPANEL_MODE_ANSWERIN_CONNECTING) {
			...
			// mRingingMuteView.setVisibility(View.VISIBLE);
		...
		//action ringingmute
		//            mRingingMuteView.setOnClickListener(new ImageView.OnClickListener() {				
		...
		//			});
		// action panel and buttons
		// mActionPanel = (VTPanelActionButton) findViewById(R.id.action_buttons);
		//   FrameLayout mBtnRecord = (FrameLayout)findViewById(R.id.btnRecord);
		//   mRecordTimeView = (TextView)this.findViewById(R.id.recordDuration);
		//   btnRecord = (ImageView) findViewById(R.id.btn_record);
		/*mBtnRecord.setOnClickListener(new FrameLayout.OnClickListener() {
            	public void onClick(View v)  {
                    ...
                    if (mApp.mAudioRecordStatus == VideoCallApp.AUDIO_RECORD_STATE.IDLE) // Avoid exception when starting audio record twice quickly.
                    {              	
                    	if (mApp.mEngineState >= VideoCallApp.ENGINE_STATE_CONNECTED) 
                    	{
                        	mApp.mAudioRecordStatus = VideoCallApp.AUDIO_RECORD_STATE.STARTING;
                        	if (MyLog.DEBUG) MyLog.d(TAG, "mAudioRecordStatus =VideoCallApp.AUDIO_RECORD_STATE.STARTING");
                                  // Check the SD card state firstly.
                        	int sdcardStatus = VTCallUtils.checkStorage();
                        	if(VTCallUtils.SDCARD_OK != sdcardStatus) {
                            	VTCallUtils.showStorageToast(sdcardStatus, VideoCallScreen.this);
                            	mApp.mAudioRecordStatus = VideoCallApp.AUDIO_RECORD_STATE.IDLE;
                            	if (MyLog.DEBUG) MyLog.d(TAG, "VTCallUtils.SDCARD_OK != sdcardStatus;mAudioRecordStatus = VideoCallApp.AUDIO_RECORD_STATE.IDLE");
                             	return;
                         	}
	                        updateVTStatusFile(IS_RECORDING);
	                        if(progressDlg==null)
	                        {
	                        	progressDlg = ProgressDialog.show(VideoCallScreen.this, "", VideoCallScreen.this.getText(R.string.please_wait_msg), true, false);
	                        }
	                        Thread task = new Thread(new StartAudioRecordTask());
                        	task.start();
                        	btnRecord.setBackgroundResource(R.drawable.cmcc_phone_vtcall_btn_icon_record_hl);
                    	}
                    } 
                    else if(mApp.mAudioRecordStatus == VideoCallApp.AUDIO_RECORD_STATE.STARTED)
                    {
                    	mApp.stopAudioRecord();		
                     	mPanelManager.showEndRecordAudioState();
                    	btnRecord.setBackgroundResource(R.drawable.cmcc_phone_vtcall_btn_icon_record);
                        updateVTStatusFile(NOT_RECORDING);
                 	}
                }
		...
		btnSwitch = (ImageView) findViewById(R.id.btn_switch);
		mBtnSwitch.setOnClickListener(new FrameLayout.OnClickListener() {
			public void onClick(View v)  {
				if(1 == android.hardware.Camera.getNumberOfCameras()){
					//MyLog.d(TAG, "only one camera on your device, can not switch camera!");
		...
		//            View mBtnShowKeyboard = findViewById(R.id.btnKeyboard);
		//            mBtnShowKeyboard.setOnClickListener(new View.OnClickListener() {
		//                public void onClick(View v) {
		//                      onShowKeyboard();
		//                }
		//            });

		View mbtnMute = findViewById(R.id.btnmute);
