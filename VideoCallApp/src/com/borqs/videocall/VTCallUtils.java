...
public class VTCallUtils {
    ...
    // important not to
    // put it in the root directory which may have a limit on the number of
    // files.
    static private boolean checkFsWritable() {
        synchronized (lockToCheckWriteAble) {
            ...
            if (!minithumbDir.isDirectory()) {
                if (!minithumbDir.mkdirs()) {
                    return false;
                }
            }
    ...
    static int checkMOInitStatus(ContentResolver cr, String number) {
        ...
        if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) { // Multiple
                                                                     // simcards
                                                                     // are
                                                                     // active
            ...
            int type = MSimTelephonyManager.getDefault().getVoiceNetworkType(0);
            if (MyLog.DEBUG)
            ...
            // check call state
            int callState0 = MSimTelephonyManager.getDefault().getCallState(0);
            int callState1 = MSimTelephonyManager.getDefault().getCallState(1);
            if (MyLog.DEBUG)
